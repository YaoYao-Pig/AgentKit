...
> If you require code content to be generated by the API-side agent (not a local stub), also enable:
>
> ```yaml
...
>
> This mode enforces:
> - no local endpoints (`localhost` / `127.x` / `0.0.0.0` / `.local`)
> - required API response `provenance` fields:
>
> ```json
> {
>   "provenance": {
